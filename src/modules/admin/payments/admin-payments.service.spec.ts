...
import { TransactionStatus, SubscriberType } from '../../payments/entities/transaction.entity';
...
describe('AdminPaymentsService', () => {
  ...
  let employerSubscriptionRepo: { count: jest.Mock; createQueryBuilder: jest.Mock };
  let talentSubscriptionRepo: { count: jest.Mock; createQueryBuilder: jest.Mock };
  ...
  beforeEach(() => {
    ...
    employerSubscriptionRepo = { count: jest.fn(), createQueryBuilder: jest.fn() };
    talentSubscriptionRepo = { count: jest.fn(), createQueryBuilder: jest.fn() };
  ...
  describe('getStats', () => {
    it('returns stat cards with computed values', async () => {
      transactionRepo.createQueryBuilder.mockReturnValue(
        chainable({ getRawOne: jest.fn().mockResolvedValue({ total: '5499.99' }) }),
      );
  ...
  describe('getRevenueChart', () => {
    ...
    it('groups revenue by employer and talent with monthly default period', async () => {
      const employerRows = [makeRow('2026-01-01', 1200), makeRow('2026-02-01', 980)];
      const talentRows = [makeRow('2026-01-01', 450)];

      transactionRepo.createQueryBuilder
        .mockReturnValueOnce(chainable({ getRawMany: jest.fn().mockResolvedValue(employerRows) }))
        .mockReturnValueOnce(chainable({ getRawMany: jest.fn().mockResolvedValue(talentRows) }));

      const result = await service.getRevenueChart({});

      expect(result.employer_revenue).toHaveLength(2);
      expect(result.employer_revenue[0]).toEqual({ period: '2026-01-01T00:00:00.000Z', amount: 1200 });
      expect(result.talent_revenue).toHaveLength(1);
      expect(result.talent_revenue[0]).toEqual({ period: '2026-01-01T00:00:00.000Z', amount: 450 });
    });

    it('passes the correct date_trunc parameter for each period', async () => {
      transactionRepo.createQueryBuilder
        .mockReturnValue(chainable({ getRawMany: jest.fn().mockResolvedValue([]) }));

      await service.getRevenueChart({ period: 'yearly' });

      const calls = transactionRepo.createQueryBuilder.mock.results;
      const sql1 = (calls[0].value as ReturnType<typeof chainable>).select.mock.calls[0][0] as string;
  ...
  describe('getEmployerPackages', () => {
    it('returns packages sorted by price ascending', async () => {
      employerPackageRepo.find.mockResolvedValue([
        { id: 'pkg-1', name: 'Free', price: '0', offer_limit: 2, features: null, is_free: true },
        { id: 'pkg-2', name: 'Paid', price: '0', offer_limit: null, features: null, is_free: false },
      ]);
      ...
      expect(employerPackageRepo.find).toHaveBeenCalledWith({ order: { price: 'ASC' } });
    });
  ...
  describe('getSubscriptions', () => {
    it('returns merged employer + talent subscriptions when no type filter', async () => {
      ...
      const employerQb = chainable({
        getRawMany: jest.fn().mockResolvedValue([
          { id: 'es-1', subscriber_name: 'Acme Corp', type: 'employer', package_tier: 'Free', monthly_price: '0', status: 'active', start_date: earlyDate, next_billing_date: null, grace_period_ends_at: null },
        ]),
        getCount: jest.fn().mockResolvedValue(1),
      });
      const talentQb = chainable({
        getRawMany: jest.fn().mockResolvedValue([
          { id: 'ts-1', subscriber_name: 'Jane Talent', type: 'talent', package_tier: null, monthly_price: '9.99', status: 'active', start_date: laterDate, next_billing_date: new Date('2026-04-01'), grace_period_ends_at: null },
        ]),
        getCount: jest.fn().mockResolvedValue(1),
      });
    ...
    it('filters by talent type only', async () => {
      ...
      const talentQb = chainable({
        getRawMany: jest.fn().mockResolvedValue([
          { id: 'ts-1', subscriber_name: 'Jane Talent', type: 'talent', package_tier: null, monthly_price: null, status: 'free', start_date: new Date(), next_billing_date: null, grace_period_ends_at: null },
        ]),
        getCount: jest.fn().mockResolvedValue(1),
      });
    ...
    it('computes days_left_in_grace for past_due employer subscriptions', async () => {
      ...
      const employerQb = chainable({
        getRawMany: jest.fn().mockResolvedValue([
          { id: 'es-1', subscriber_name: 'Acme Corp', type: 'employer', package_tier: 'Paid', monthly_price: '49.99', status: 'past_due', start_date: new Date(), next_billing_date: new Date(), grace_period_ends_at: futureGrace },
        ]),
        getCount: jest.fn().mockResolvedValue(1),
      });
    ...
    it('returns null days_left_in_grace for talent subscriptions', async () => {
      ...
      const talentQb = chainable({
        getRawMany: jest.fn().mockResolvedValue([
          { id: 'ts-1', subscriber_name: 'Jane Talent', type: 'talent', package_tier: null, monthly_price: null, status: 'active', start_date: new Date(), next_billing_date: null, grace_period_ends_at: null },
        ]),
        getCount: jest.fn().mockResolvedValue(1),
      });
    ...
    it('returns empty items when no subscriptions match', async () => {
      employerSubscriptionRepo.createQueryBuilder.mockReturnValue(
        chainable({ getRawMany: jest.fn().mockResolvedValue([]), getCount: jest.fn().mockResolvedValue(0) }),
      );
  ...
  describe('getTransactions', () => {
    ...
    it('filters by transaction status', async () => {
      ...
      expect(qb.andWhere).toHaveBeenCalledWith('txn.status = :status', { status: 'failed' });
    });
  ...
  describe('getTalentSubscriptionSummary', () => {
    it('returns counts and distinct price', async () => {
      talentSubscriptionRepo.count.mockResolvedValueOnce(45).mockResolvedValueOnce(8);
      talentSubscriptionRepo.createQueryBuilder.mockReturnValue(
        chainable({ getRawOne: jest.fn().mockResolvedValue({ price: '9.99' }) }),
      );
    ...
    it('returns null monthly_price when no paid subscriptions exist', async () => {
      talentSubscriptionRepo.count.mockResolvedValueOnce(0).mockResolvedValueOnce(0);
      talentSubscriptionRepo.createQueryBuilder.mockReturnValue(
    ...
    it('counts only ACTIVE talent subscriptions', async () => {
      talentSubscriptionRepo.count.mockResolvedValueOnce(30).mockResolvedValueOnce(5);
      talentSubscriptionRepo.createQueryBuilder.mockReturnValue(
        chainable({ getRawOne: jest.fn().mockResolvedValue(null) }),
      );
